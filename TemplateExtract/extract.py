...
LTP_DATA_DIR='F:\jupyter-notebook\extraction\data\model'  # ltp模型目录的路径
# user_dict = 'F:\jupyter-notebook\extraction\data\model\lexicon.txt'
...
data = []
text_dict = {}
file1 = open('./TemplateExtract/file1.text', encoding='utf-8')
file4 = open('./TemplateExtract/file4.text', 'w', encoding='utf-8')
file5 = open('./TemplateExtract/file5.text', 'w', encoding='utf-8')
count1=0

#提取案情
#所需文件有两个，legaltext放的是文书->file1里是案情描述
def data_process():
    # file1 = open('file1.text', 'w', encoding='utf-8')
    f = open('legaltext.json', encoding='utf-8')
    ...
    for dict_data in data:
        if len(dict_data['诉请']):
            text = ""
            for i in dict_data['诉请']:
                text = text + i
        elif '案件要素' in dict_data:
            if dict_data['案件要素'][0]["Value"] != "[]":
                text = dict_data['案件要素'][0]["Value"]
        else:
            text = "None"
        file1.write(text + '\n')
    file1.close()
...
#提取日期
#file1是案情描述->file2是提取出的日期
def findDate():
    # file1 = open('file1.text', encoding='utf-8')
    file2 = open('file2.text', 'w', encoding='utf-8')
    for line in file1:
...
def findThing(str):
    ...
    # 匹配"将...拿走"
    temp = re.findall(r"(?<=将|见).*(?=拿走|脱下|盗出|盗转|剪断|盗走|拿入|装入|窃走|搬到|窃回|窃取|分多次转至|未锁好|未上锁|带至|带入|藏匿|转走|带进|装在|转入|提现|以人民币|开锁骑走|放出卖于|运回)", str)
    # 匹配"试图剪断...的环形锁"
    ...
    if len(temp0) == 0 and len(temp) == 0 and len(temp1)==0 and len(temp2)==0 and len(temp3)==0 and len(temp4)==0 and len(temp5)==0:
        if "窃" in str:
...
def findAddress2(str, str1):
    ...
    if "到" in str:
        ...
        array.append(findAddress(add[1]))  # 地点
        return array
    elif "位于" in str:
        add = str1.split("位于")
        array.append(add[0])  # 人物
...
def findAddress3(str):
    ...
    if "到" in str and "到案" not in str:
        ...
        if "期间" not in str and "授意" not in str and '身上' not in str:      # 在XXX期间
            add = str.split("在")
            return findAddress(add[1])
    elif "进入" in str:
        add = str.split("进入")
...
def processPerson(str):
    str = re.sub('(先后|使用|未经|来到|趁|通过|持票|三次|两次|酒后|游荡|采取|结伙|谎称|尾随|明知|见|'
                 ...
                 '|至|一同|分别|作为|发现|利用|共同|商议|进入|窃取|盗窃|在|相识|窃得|盗刷|到|攀|行|均系|让|原系|化名|遂|应聘|自|从|步|佯装).*', '',
               str)  # 去除掉句子里的"在xxx授意下"
    final=[]
...
def NER2(str):
    ...
    length = len(words)
    for i in range(0, length):
        if words[i]=="狗" or words[i]=="鹅"  or words[i]=="Iphone6S"  or words[i]=="Iphone7PLUS" or words[i]=="VIVO" or words[i]=="OPPO" or words[i]=="OPPOA5":
            postags[i]="o"
...
def extract(line):
    ...
    sen_count=0
    if len(re.findall(r".*人民检察院指控",line))!=0:
        place=re.findall(r".*人民检察院指控",line)[0]
        place=place.replace("人民检察院指控", "")
    elif len(re.findall(r".*铁路运输检察院指控",line))!=0:
        place=re.findall(r".*铁路运输检察院指控",line)[0]
        place=place.replace("铁路运输检察院指控", "")
    ...
    for value in strlist:
        ...
        if ((sen_count==1 and "本院认为" not in value) or (len(re.findall(r"(\d{4}年\d{1,2}月\d{1,2}日|\d{4}年\d{1,2}月中旬|\d{4}年\d{1,2}月上旬|\d{4}年\d{1,2}月下旬|\d{4}年\d{1,2}月|\d{4}年|\d{1,2}时\d{2}分|某年某月某日|\d{1,2}时许|同年\d{1,2}月\d{1,2}日|次日)", value)) != 0 )and(
                "盗" in value or "窃" in value or "拿走" in value or "携赃" in value or "作案" in value or "搬到" in value or "试图剪断" in value or "偷" in value or "装入" in value or "撬开" in value or "电表" in value or "装在" in value or "吸取被害人" in value or "开锁骑走" in value) \
                ...
                and "价格卖掉" not in value and "予以否认"not in value and "被公安人员当场人赃俱获" not in value and "执勤民警发现" not in value and "将被盗电动车归还" not in value and "被抓获到案" not in value and "私接线路的窃电量" not in value):  # 如果句子中包含时间，则可能为盗窃事件
            ...
            text_dict["人物"] = []
            for j in sub:
                dateArray = re.findall(r"(\d{4}年\d{1,2}月\d{1,2}日|\d{4}年\d{1,2}月中旬|\d{4}年\d{1,2}月上旬|\d{4}年\d{1,2}月下旬|\d{4}年\d{1,2}月|\d{1,2}月\d{1,2}日|\d{4}年|\d{1,2}月|\d{1,2}日|次日|\d{1,2}时\d{1,2}分|某年某月某日|\d{1,2}时许|同年\d{1,2}月\d{1,2}日)",j)
                if len(dateArray) != 0:
                ...
                if "被告" in j and flag_name == 0:  # 尚未找到被告
                    ...
                    if "于" in name[1] and "停放于" not in name[1]:     # 被告人xxx于某年某月在某地盗窃某物,已经提取到时间
                        ...
                        if findAddress2(j, name[1])[0] != None:  # 被告人xxx在某地盗窃某物
                            ...
                            flag_name = 1
                        if findAddress2(j, name[1])[1] != None:
                ...
                if text_dict["人物"]==[]:
                    temp=re.findall('[\u4e00-\u9fa5]{3}伙同[\u4e00-\u9fa5]{3}', j)#[\u4e00-\u9fa5]表示至少匹配一个汉字
                ...
                if text_dict["地点"] =='' and findAddress4(j)!= None:
                    ...
                    flag = 1
                if text_dict["地点"] =='' and place!='':#实在不行在哪审的就在哪做的案
                    text_dict["地点"] = place
                    flag = 1
            ...
            if text_dict["时间"].startswith("次日") and len(prefix)!=0:
                text_dict["时间"]=re.findall(r"(\d{4}年\d{1,2}月)", time)[0]+str(int(re.findall(r"(\d{1,2}日)", time)[0].split("日")[0])+1)+"日"+text_dict["时间"].split("次日")[1]
            ...
            if text_dict["时间"]=='' or text_dict["物品"] == [] or text_dict["地点"]=='' :
                #print(1111)
                continue
            else:
                #每句话是一个字典，每个文书是字典数组
